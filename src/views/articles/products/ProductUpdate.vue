<template>
  <VForm :initial-values="intitalValues" @submit="onProductSubmit" :validation-schema="product">
    <div class="card-header">
      <h3 class="card-title">Update new Product</h3>
...
export default defineComponent({
  ...
  methods: {
    getProduct() {
      ApiService.get("products/" + this.id + '?with[]=CategoryParameters&')
        .then(({ data }) => {
          this.categoryValue = data.data.product_category_id;
          this.onCategoryChange(this.categoryValue);
          this.intitalValues = data.data;
          let parameters = {};
          data.data.category_parameters.forEach(param => { 
            parameters = { ...parameters , ...{ [`paramid_${param.id}`] : param.pivot.value } };
          });
          this.intitalValues.parameters = parameters;
          this.isStandard = data.data.type === "standard" ? true : false;
    ...
    onProductSubmit(values) {
     ...
     update(dataToSave) {
      this.isLoading = true;
      return  ApiService.put("products/"+this.id, dataToSave)
        .then( async ({ data }) => {
          this.isLoading = false;   
          console.log(this.intitalValues)  
          Swal.fire({
            position: "top-end",
            icon: "success",
    ...
    onCategoryChange(id) {
      ...
      else this.isParameterLoading = false;
    },
  ...
  async created() {
    try {
      ...
      await Promise.all([
        this.getProduct(),
        this.getCategories("?company_id=1"),
      ]);
      this.isLoading = false;
