    ...
    <div class="card-header">
      ...
      <div class="card-toolbar d-flex justify-content-end">
        <div class="return-button p-1">
          <button
            @click="$router.push('/articles/sale_prices')"
            type="button"
                  ...
                  <tbody>
                      <tr v-for="selectedArticle in  selectedArticles">
                          <td class="px-2">
                              <button
                                type="button"
                                class="btn btn-sm btn-danger p-2 text-center"
...
export default defineComponent({
  ...
  methods: {
    ...
    async onFormSubmit(values) {
      this.isLoading = true;
      let isPricesEmpty = false;
      this.selectedArticles.map( (article) => {
          if( article.new_price == 0 || article.new_price == null ) {
            isPricesEmpty = true;
            return;
          }
      })
      if( isPricesEmpty ){
        Swal.fire({
          position: "top-end",
          icon: "error",
          title: "Please fill all articles prices",
      ...
      values.products = this.selectedArticles.filter((article) => article.type_article == 'product')
                                              .map((art) => { return {price: art?.new_price, object_id: art?.id }});
    ...
    update(values) {
      return ApiService.put("sale_prices/"+ this.id, values)
        .then(({ data }) => {
          this.isLoading = false;
          Swal.fire({
            position: "top-end",
    ...
    getIntitalValues(){
      return ApiService.get("sale_prices/"+ this.id)
        .then(({ data }) => {
          const result = data.data;
          if(!result.all_customer) this.customers_ids = result.sale_price_customers.map(({ customer_id }) => customer_id );
          if(!result.all_center) this.centers_ids = result.sale_price_centers.map(({ center_id }) => center_id );
          let products = result.hasOwnProperty('products') ? result?.products.map( product => { 
            return {
              object_id: product?.id,
              name: product?.name,
              reference: product?.code,
              origine_price: product?.price,
          ...
          let packs = result.hasOwnProperty('packs') ? result?.packs.map( pack => { 
            return {
              ...
              origine_price: pack?.price,
              new_price: pack?.pivot?.price,
              type_article: 'pack'
            }
          }) : [];
          let forfaits = result.hasOwnProperty('forfaits') ? result.forfaits.map( forfait => { 
            return {
              ...
              origine_price: forfait?.price,
              new_price: forfait?.pivot?.price,
    ...
    getCategories(){
      ApiService.get("sale_prices/get_product_categories")
        .then(({ data }) => {
    ...
    getArticles(){
      ...
      ApiService.get("sale_prices/get_articles?"+queryString+"&")
        .then(({ data }) => {
