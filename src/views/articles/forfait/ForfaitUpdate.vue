...
export default defineComponent({
  ...
  methods: {
      ...
      return ApiService.get("forfaits/get_datacar?brand_id="+this.brandValue
        ...
        .then(({ data }) => {
          ...
          Swal.fire({
            position: "top-end",
            icon: "success",
            title: "Datacar has been added successfully",
            showConfirmButton: false,
            timer: 1500,
          });
        })
    ...
    update(forfait) {
      return ApiService.put("forfaits/" + this.id, forfait)
        .then(({ data }) => {
          this.isLoading = false;
          Swal.fire({
            position: "top-end",
    ...
    getForfait(){
      return ApiService.get("forfaits/"+ this.id)
        .then(({ data }) => {
          const forfait = data.data;
          this.nature_id = forfait.nature_id;
          this.onSelectNature(this.nature_id);
          this.sub_nature_id = forfait.sub_nature_id;
          this.datacar = forfait.data_car;
          this.intitalValues = forfait
        })
        .catch(({ error }) => {
          console.log(error);
        });
