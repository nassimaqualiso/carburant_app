...
export default defineComponent({
  ...
  methods: {
    getDataListing() {
      this.$emit("getDataListing", "vehicle_brands");
    },
    ...
    getDataElement(value) {
      return ApiService.get(this.moduleName + "/" + value)
        .then(({ data }) => {
          ...
          console.log("brands ot edited", data);
        })
        .catch(({ error }) => {
          console.log(error);
          this.isModalLoading = false;
      ...
      return ApiService.post(this.moduleName, element)
        .then(({ data }) => {
          if (data.success) {
            ...
            this.isModalLoading = false;
            this.$refs.brandDataTable.getDataListing();
            Swal.fire({
              position: "top-end",
              icon: "success",
        ...
        .catch(({ error }) => {
          ...
          Swal.fire({
            position: "top-end",
            icon: "error",
            title: "brand failed to be created",
            showConfirmButton: false,
            timer: 1500,
          });
        });
