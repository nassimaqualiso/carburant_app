<template>
  <!--begin::Form-->
  <VForm @submit="OnCentersubmit" :validation-schema="center">
...
export default defineComponent({
  ...
  methods: {
    ...
    store(center) {
      return ApiService.post("centers", center)
        .then(({ data }) => {
  ...
  async created() {
    try {
      await Promise.all([this.getCompanies("pluck=name,id")]);

      this.isLoading = false;
      this.center = Yup.object().shape({
        name: Yup.string().min(4).required().label("name"),
        rc: Yup.string().max(50).label("RC"),
        ice: Yup.string().max(50).label("ICE"),
        idfiscale: Yup.string().max(50).label("idfiscale"),
        patent: Yup.string().label("patent"),
        phone1: Yup.string().label("Phone1"),
        phone2: Yup.string().label("Phone2"),
        manager_phone: Yup.string().label("Phone2"),
        manager: Yup.string().label("manager"),
        manager_email: Yup.string().email().label("Email"),
